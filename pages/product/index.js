...
Page({
  data: {
    ...
    intoView: "",
    showShare: false,
    shareOptions: [{ name: "微信", icon: "wechat", openType: "share" }],
  ...
  async getProduct(params) {
    ...
    this.setData({
      product: data,
      carouselList: data.carousel_picture,
      carouselCount: data.carousel_picture.length,
    });
  },
  async getCartCount() {
    const { productId } = this.data;
  ...
  async getProductList() {
    ...
    const { code, data, msg } = await getRecommendProduct(params);
    let products;
    if (currentPage === 1) {
      this.setData({
